 ...
 */
package component.nhaCungCapComponent;
 ...
 * @author mansh
 */
public class UpdateNCC extends javax.swing.JDialog {

    private ToolBarAndTableNCC tblNCC;
    private NhaCungCapDTO selectedNCC;

    public UpdateNCC(java.awt.Frame parent, boolean modal, ToolBarAndTableNCC tblNCC) {
        super(parent, modal);
        initComponents();
        this.tblNCC = tblNCC;
        showOldValue();
    ...
    private void showOldValue() {
        String idNCC = tblNCC.getSelectedItemId();
        NhaCungCapDTO ncc = new NhaCungCapDTO();
        ncc.setMaNCC(idNCC);
        this.selectedNCC = NhaCungCapBUS.getInstance().getById(ncc);
        tenTxt.setText(selectedNCC.getTen());
        diaChiTxt.setText(selectedNCC.getDiaChi());
        sdtTxt.setText(selectedNCC.getSoDienThoai());
        emailTxt.setText(selectedNCC.getEmail());
    }
    ...
    private void jButton2MouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jButton2MouseClicked
        ...
        String errors = NhaCungCapBUS.getInstance().validateData(ten, diaChi, sdt, email);
        if (errors == null || errors.isEmpty()) {
            NhaCungCapDTO ncc = NhaCungCapBUS.getInstance().createUpdatedNCC(this.selectedNCC.getMaNCC(), ten, diaChi, sdt, email);
            int result = NhaCungCapBUS.getInstance().update(ncc);
            if (result > 0) {
                JOptionPane.showMessageDialog(this, "Cập nhật NCC thành công!", "Thành công!", JOptionPane.INFORMATION_MESSAGE);
                tblNCC.refreshDataListNCC();
                tblNCC.loadDataNCC(tblNCC.getListNCC());
                this.dispose();
            } else {
                JOptionPane.showMessageDialog(this, "Cập nhật NCC thất bại, lỗi hệ thống!", "Thất bại", JOptionPane.ERROR_MESSAGE);
            }
        } else {
            JOptionPane.showMessageDialog(this, errors, "Cập nhật NCC thất bại!", JOptionPane.INFORMATION_MESSAGE);
        }
