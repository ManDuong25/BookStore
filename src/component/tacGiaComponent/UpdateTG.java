 ...
 */
package component.tacGiaComponent;
 ...
 * @author mansh
 */
public class UpdateTG extends javax.swing.JDialog {

    private ToolBarAndTableTacGia tblTG;
    private TacGiaDTO selectedTG;

    public UpdateTG(java.awt.Frame parent, boolean modal, ToolBarAndTableTacGia tblTG) {
        super(parent, modal);
        initComponents();
        this.tblTG = tblTG;
        showOldValue();
    ...
    private void showOldValue() {
        String idTG = tblTG.getSelectedItemId();
        TacGiaDTO tg = new TacGiaDTO();
        tg.setMaTacGia(idTG);
        this.selectedTG = TacGiaBUS.getInstance().getById(tg);
        tenTxt.setText(selectedTG.getTen());
        diaChiTxt.setText(selectedTG.getDiaChi());
        sdtTxt.setText(selectedTG.getSdt());
        emailTxt.setText(selectedTG.getEmail());
    }
    ...
    private void jButton2MouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jButton2MouseClicked
        ...
        String errors = TacGiaBUS.getInstance().validateData(ten, diaChi, sdt, email);
        if (errors == null || errors.isEmpty()) {
            TacGiaDTO tg = TacGiaBUS.getInstance().createUpdatedTG(this.selectedTG.getMaTacGia(), ten, diaChi, sdt, email);
            int result = TacGiaBUS.getInstance().update(tg);
            if (result > 0) {
                JOptionPane.showMessageDialog(this, "Cập nhật TG thành công!", "Thành công!", JOptionPane.INFORMATION_MESSAGE);
                tblTG.refreshDataListTG();
                tblTG.loadDataTG(tblTG.getListTG());
                this.dispose();
            } else {
                JOptionPane.showMessageDialog(this, "Cập nhật TG thất bại, lỗi hệ thống!", "Thất bại", JOptionPane.ERROR_MESSAGE);
            }
        } else {
            JOptionPane.showMessageDialog(this, errors, "Cập nhật TG thất bại!", JOptionPane.INFORMATION_MESSAGE);
        }
