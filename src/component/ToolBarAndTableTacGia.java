...
import component.tacGiaComponent.AddTG;
import component.tacGiaComponent.UpdateTG;
 ...
 * @author mansh
 */
public class ToolBarAndTableTacGia extends javax.swing.JPanel {

    private MainView mv;
    private ArrayList<TacGiaDTO> listTG = TacGiaBUS.getInstance().getAll();

    public ToolBarAndTableTacGia(MainView mv) {
        initComponents();
        this.mv = mv;
        filterConditionCbx.addItem("Mặc định");
        filterConditionCbx.addItem("Theo mã TG");
    ...
    public void loadDataTG(ArrayList<TacGiaDTO> listTG) {
...
//            System.out.println(sach);
        ...
        tgTbl.setModel(defaultTableModel);
        tgTbl.setRowHeight(50);
        tgTbl.getTableHeader().setReorderingAllowed(false);
    }

    public String getSelectedItemId() {
        DefaultTableModel dtm = (DefaultTableModel) tgTbl.getModel();
        int selectedRowIndex = tgTbl.getSelectedRow();
        String id = "";
    ...
    public void deleteItem() {
        ...
        if (id.isEmpty() || id == null) {
            JOptionPane.showMessageDialog(mv, "Vui lòng chọn tác giả muốn xoá!", "Thất bại", JOptionPane.ERROR_MESSAGE);
        } else {
            // Hiển thị hộp thoại xác nhận
            int confirm = JOptionPane.showConfirmDialog(mv, "Bạn có chắc chắn muốn xóa tác giả này không?", "Xác nhận", JOptionPane.YES_NO_OPTION);
            // Kiểm tra người dùng nhấn Yes hay không
            if (confirm == JOptionPane.YES_OPTION) {
                TacGiaDTO tg = new TacGiaDTO();
                tg.setMaTacGia(id);
                int result = TacGiaBUS.getInstance().delete(tg);
                if (result > 0) {
                    JOptionPane.showMessageDialog(mv, "Xoá tác giả thành công!", "Thành công", JOptionPane.ERROR_MESSAGE);
                    this.listTG = TacGiaBUS.getInstance().getAll();
                    loadDataTG(listTG);
                }
            }
        }
        ...
        try {
            ...
            if (userSelection == JFileChooser.APPROVE_OPTION) {
                ...
                for (int row = 1; row <= sheet.getLastRowNum(); row++) {
                    ...
                    Cell cellMaTG = excelRow.getCell(0);
                    String maTG;
                    if (cellMaTG.getCellType() == CellType.NUMERIC) {
                        // Handle numeric value, such as formatting it to string
                        double numericValue = cellMaTG.getNumericCellValue();
                        maTG = String.valueOf(numericValue);
                    } else {
                        maTG = cellMaTG.getStringCellValue().trim();
                    }
                    ...
                    String errors = TacGiaBUS.getInstance().validateData(maTG, ten, diaChi, sdt, email);
                    if (errors == null || errors.isEmpty()) {
                        int flag = 0;
                        for (TacGiaDTO tgInDB : this.listTG) {
                            if (tgInDB.getTen().equals(ten) && tgInDB.getDiaChi().equals(diaChi) && tgInDB.getSdt().equals(sdt) && tgInDB.getEmail().equals(email)) {
                                flag = 1;
                            }
                        }
                        if (flag == 0) {
                            TacGiaDTO tg = TacGiaBUS.getInstance().createNewTG(ten, diaChi, sdt, email);
                            int result = TacGiaBUS.getInstance().add(tg);
                            if (result > 0) {
                                JOptionPane.showMessageDialog(this, "Đã thêm thành công TG có mã là: " + tg.getMaTacGia(), "Thành công!", JOptionPane.INFORMATION_MESSAGE);
                                this.listTG = TacGiaBUS.getInstance().getAll();
                            }
                        }
                    } else {
    ...
    public ArrayList<TacGiaDTO> filterByMaTG(ArrayList<TacGiaDTO> list) {
        if (filterConditionCbx.getSelectedIndex() == 1) {
            ...
            if (!filterValueTxtFld2.getText().isEmpty() && filterValueTxtFld2.getText() != null) {
                for (TacGiaDTO tg : list) {
                    if (tg.getMaTacGia().contains(filterValueTxtFld2.getText())) {
                        result.add(tg);
                    }
                }
                return result;
    ...
    public ArrayList<TacGiaDTO> filterBySDT(ArrayList<TacGiaDTO> list) {
        if (filterConditionCbx.getSelectedIndex() == 2) {
            ArrayList<TacGiaDTO> result = new ArrayList<TacGiaDTO>();
            if (!filterValueTxtFld2.getText().isEmpty() && filterValueTxtFld2.getText() != null) {
                for (TacGiaDTO tg : list) {
                    if (tg.getSdt().contains(filterValueTxtFld2.getText())) {
                        result.add(tg);
                    }
                }
                return result;
    ...
    public ArrayList<TacGiaDTO> filterByEmail(ArrayList<TacGiaDTO> list) {
        if (filterConditionCbx.getSelectedIndex() == 3) {
            ArrayList<TacGiaDTO> result = new ArrayList<TacGiaDTO>();
            if (!filterValueTxtFld2.getText().isEmpty() && filterValueTxtFld2.getText() != null) {
                for (TacGiaDTO tg : list) {
                    if (tg.getEmail().contains(filterValueTxtFld2.getText())) {
                        result.add(tg);
                    }
                }
                return result;
    ...
    public ArrayList<TacGiaDTO> ascendingSort(ArrayList<TacGiaDTO> list) {
        int sortIndex = sortCbx.getSelectedIndex();
        int filterIndex = filterConditionCbx.getSelectedIndex();

        Comparator<TacGiaDTO> comparator = null;

        if (sortIndex == 1 && filterIndex == 1) {
            comparator = Comparator.comparing(TacGiaDTO::getMaTacGia);
        } else if (sortIndex == 1 && filterIndex == 2) {
            comparator = Comparator.comparing(TacGiaDTO::getSdt);
        } else if (sortIndex == 1 && filterIndex == 3) {
            comparator = Comparator.comparing(TacGiaDTO::getEmail);
        }
    ...
    public ArrayList<TacGiaDTO> descendingSort(ArrayList<TacGiaDTO> list) {
        int sortIndex = sortCbx.getSelectedIndex();
        int filterIndex = filterConditionCbx.getSelectedIndex();
        ...
        if (sortIndex == 2 && filterIndex == 1) {
            comparator = Comparator.comparing(TacGiaDTO::getMaTacGia).reversed();
        } else if (sortIndex == 2 && filterIndex == 2) {
            comparator = Comparator.comparing(TacGiaDTO::getSdt).reversed();
        } else if (sortIndex == 2 && filterIndex == 3) {
            comparator = Comparator.comparing(TacGiaDTO::getEmail).reversed();
        }
    ...
    public void refreshDataListTG() {
        this.listTG = TacGiaBUS.getInstance().getAll();
    }

    public ArrayList<TacGiaDTO> getListTG() {
        return this.listTG;
    }
    ...
    private void initComponents() {
        ...
        tgTbl.setModel(new javax.swing.table.DefaultTableModel(
            new Object [][] {
                {null, null, null, null},
                {null, null, null, null},
                {null, null, null, null},
        ...
        jScrollPane2.setViewportView(tgTbl);

        jTabbedPane1.addTab("Danh Sách TG", jScrollPane2);
    ...
    private void addBtnMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_addBtnMouseClicked
        ...
        AddTG addTG = new AddTG(mv, true, this);
        addTG.setLocationRelativeTo(mv);
        addTG.setVisible(true);
    ...
    private void exportExcelFileBtnMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_exportExcelFileBtnMouseClicked
        ...
        ExcelUtil.saveFile(mv, tgTbl, "tg");
    ...
    private void readExcelBtnMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_readExcelBtnMouseClicked
        ...
        String[] expectedHeaders = {"Mã tác giả", "Tên", "Địa chỉ", "Số điện thoại", "Email"};
    ...
    private void jButton2MouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jButton2MouseClicked
        loadDataTG(filterByEmail(filterByMaTG(filterBySDT(ascendingSort(descendingSort(listTG))))));
    ...
    private void updateBtnMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_updateBtnMouseClicked
        ...
        if (id.isEmpty() || id == null) {
            JOptionPane.showMessageDialog(this, "Vui lòng chọn TG muốn cập nhật!", "Thất bại", JOptionPane.ERROR_MESSAGE);
        } else {
            UpdateTG updateTG = new UpdateTG(mv, true, this);
            updateTG.setLocationRelativeTo(mv);
            updateTG.setVisible(true);
        }
    ...
    private javax.swing.JTable tgTbl;
